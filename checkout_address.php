...
if (!($_SERVER["REQUEST_METHOD"] == "POST" && isset($_SESSION['mmb_id']))) {
	$_SESSION['status'] = 'ไม่พบสินค้าในตะกร้า โปรดลองใหม่อีกครั้ง';
...
<body class="home">
	...
	<div class="checkout-area mb-70">
		<div class="container">
			<div class="row">
				<div class="col-12">
					<form id="orderForm" method="POST" onsubmit="return validateForm()" class="parsley-examples needs-validation" novalidate action="order_add.php">
						<div class="row">
											...
											<div class="checkout-form-list">
												...
												?>
													...
													?>
														...
														<p>ยังไม่มีที่อยู่จัดส่ง <a href="my-account-address.php">กรุณาเพิ่มที่อยู่จัดส่งก่อนทำรายการ</a></p>
									...
									<div class="row">
										<div class=" col-lg-12">
											...
											<div class="buttons-cart">
												<a href="cart.php" style="background-color: gray;">ย้อนกลับ</a>
											</div>
										</div>
									</div>
								</div>
							...
							<div class="col-lg-6 col-md-12 col-12">
								<div class="your-order">
										...
										<table>
												...
												if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_SESSION['mmb_id'])) {
													...
													foreach ($_SESSION['strProductID'] as $i => $productID) {
														...
														if (!empty($productID)) {
															...
															$product_discount += ($row['prd_discount'] / 100 * $row['prd_price']);
												?>
															<tr class="cart_item">
																<td class="product-name">
																	<?= $productName ?>
																</td>
																<td><strong class="product-quantity"> × <?= $quantity ?></strong></td>
																<td class="product-total">
																	<span class="amount">฿<?= number_format($productPrice, 2) ?></span>
																</td>
															</tr>
												<?php
														}
													...
													// นับรวมราคาสุทธิ
													$totalPrice = $totalProductPrice + $shippingPrice - $discoutCoin - $product_discount;
												} else {
											...
											<tfoot>
												...
												<tr class="order-total">
													...
													<td colspan="2">
														<h2><span class="amount" style="color:#f8902c;"><?= $totalProductCoins ?></span></h2>
													</td>
												</tr>
												<tr class="order-total">
													<th>
													...
													<td colspan="2">
														<h2><span class="amount" style="color: gray;">- ฿<?= number_format($totalProductPrice - $_POST['finalPrice'], 2) ?></span></h2>
													</td>
												</tr>
												<tr class="order-total">
													<th>
												...
												<tr class="order-total">
													...
													<td colspan="2"><strong><span class="amount" style="color: black;">฿<?= number_format($_POST['finalPrice'], 2) ?></span></strong>
													</td>
												</tr>
											</tfoot>
										</table>
									...
									<div class="payment-method">
										...
										<input type="text" hidden name="ord_coin" value="<?= $totalProductCoins ?>">
										<input type="text" hidden name="payment" value="<?= $paymentMethod ?>">
										<input type="text" hidden name="shipping" value="<?= $shippingMethod ?>">
										<input type="text" hidden name="discountOutput" value="<?= $discoutCoin ?>">
										<input type="text" hidden name="totalPrice" value="<?= number_format($_POST['finalPrice'], 2) ?>">
										<div class="order-button-payment">
											<input type="submit" value="ยืนยันคำสั่งซื้อ" onclick="return validateForm()">
										</div>
	...
	<script>
		document.getElementById("office").addEventListener("change", function() {
			if (this.checked) {
				...
				var firstname = document.getElementById("mmb_firstname").value;
				var lastname = document.getElementById("mmb_lastname").value;
				var detail = document.getElementById("addr_detail").value;
				var provin = document.getElementById("addr_provin").value;
				var amphu = document.getElementById("addr_amphu").value;
				var postal = document.getElementById("addr_postal").value;
				var phone = document.getElementById("addr_phone").value;

				// กำหนดค่าของ radio button
				var addressValue = firstname + " " + lastname + " " + detail + " " + provin + " " + amphu + " " + postal + " " + phone;
				document.getElementById("address_value").value = addressValue;
			}
		});
	</script>
