...
<body class="home-1">
	...
	<div class="cart-main-area mb-70">
		<div class="container">
			<div class="row">
				...
				// Select data FROM bk_ord_orders table based on ord_id
				$selectOrderSQL = "SELECT * FROM bk_ord_orders WHERE ord_id = $ord_id";
				$selectOrderResult = mysqli_query($proj_connect, $selectOrderSQL);
				$orderData = mysqli_fetch_assoc($selectOrderResult);

				// Select payment information FROM bk_ord_payment table based on ord_payment
				$pay_id = $orderData['ord_payment'];
					...
					<tbody>
						<tr class="cart_item">
							...
							<td class="product-total">
								<span class="amount">
									<h6><?= number_format($orderData['ord_amount'], 2) . ' ' ?> บาท</h6>
								</span>
							</td>
						</tr>
						<tr>
				...
				<div class="col-lg-6 col-md-6 col-12">
					<div class="wc-proceed-to-checkout">
						<ul>
							...
							<li><a href="#" id="updateCartLink" class="cancel-link" style="background: red;" onclick="confirmCancellation(<?= $orderData['ord_id'] ?>)">ยกเลิกรายการ</a></li>
						</ul>
					</div>
				</div>
				<div class="col-lg-6 col-md-6 col-12">
					<div class="wc-proceed-to-checkout">
						<ul>
							<!-- <button type="submit">อัปเดตตะกร้า</button> -->
							<li><a href="notify.php?ord_id=<?= $orderData['ord_id'] ?>" id="updateCartLink">แจ้งชำระเงิน</a></li>
						</ul>
					</div>
				</div>
			</div>
	...
	<!-- ยกเลิกรายการ -->
	<script>
		function confirmCancellation(ordId) {
			// ให้แสดง confirm box
			...
			if (confirmResult) {
				...
				window.location.href = "order_cancel.php?ord_id=" + <?= $ord_id ?>;
			} else {
