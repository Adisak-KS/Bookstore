										...
										<div class="tab-pane fade show active" id="download" role="tabpanel">
											<div class="myaccount-content">
												...
												<div class="myaccount-table table-responsive text-center">
													...
													$ord_sql = "SELECT * FROM bk_ord_orders WHERE mmb_id = {$_SESSION['mmb_id']} ORDER BY ord_date DESC";

													$ord_result = mysqli_query($proj_connect, $ord_sql);

													if (mysqli_num_rows($ord_result) > 0) {
													?>
														<table class="table table-bordered">
															...
															<tbody>
																<?php
																while ($ord_row = mysqli_fetch_assoc($ord_result)) {
																...
																?>
																	<tr>
																		...
																		<td><?php
																			if ($ord_row['ord_detail'] != '-') {

																			?>
																				<a href="https://ems.thaiware.com/<?= $ord_row['ord_detail'] ?>" target="_blank"><?= $ord_row['ord_detail'] ?></a>
																			<?php
																			}else{
																				echo '-';
																			}
																		...
																		<td><?= $ord_row['ord_amount'] ?></td>
																		<td><?= $ord_row['ord_status'] ?></td>
																		<td>
																			<?php
																			if (($ord_row['ord_status'] == 'รอการชำระเงิน') || ($ord_row['ord_status'] == 'การชำระเงินไม่ถูกต้อง')) {
																			?>
																				<a href="payment.php?ord_id=<?php echo $ord_row['ord_id']; ?>" class="btn btn-sqr"><i class="fa fa-eye"></i> รายละเอียด</a>
																			<?php
																			} else {
																			?>
																				
																				<a href="order_detail.php?ord_id=<?php echo $ord_row['ord_id']; ?>" class="btn btn-sqr"><i class="fa fa-eye"></i> รายละเอียด</a>
																			<?php
																			}
																			?>
																		</td>
